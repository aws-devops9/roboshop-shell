...
curl -o /tmp/user.zip https://roboshop-builds.s3.amazonaws.com/user.zip &>> $LOG # -o is used for overwrite
VALIDATE $? "Downloading User Application"

cd /app &>> $LOG
VALIDATE $? "Goto App Directory"

unzip -o /tmp/user.zip &>> $LOG # -o is used for overwrite
...
mongo --host mongodb.learndevops.space </app/schema/user.js &>> LOG
